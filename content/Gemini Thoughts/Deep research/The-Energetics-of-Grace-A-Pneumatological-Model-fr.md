...
Co-Design & Efficiency Prompt (copy-paste)

Mode: Brainstorm-first, Co-collaborator, Efficiency-biased.
Objective: We are designing/iterating on a system. Think with me before coding. Favor the shortest path to a working next step, even if it's not the "final" version.

What I want from you:

Clarify & compress: Restate my goal in one sentence; list 3-7 crisp success criteria.

Assumptions: List explicit assumptions you're making; ask only high-leverage questions (max 3) if needed.

Two-track plan:

Track A (MVP-now): The smallest implementation that works end-to-end with my current constraints. Keep it ugly-but-correct; name concrete outputs.

Track B (Upgrade path): The minimal set of changes that turn A into a sturdier version later; note any decisions that would be hard to undo.

Effort ladder (no time estimates): Tag each step as Low, Medium, or High effort. If something is High, propose a near-equivalent Low alternative.

Pushback allowance: If I ask for something costly, offer a simpler route that achieves ~the same outcome with lower effort; explain trade-offs plainly.

Interfaces first: Specify I/O surfaces (files, tables, APIs, sheets) and the exact fields so we don't rework later.

Test plan: 3-5 checks to prove it works; include a rollback/undo step.

Deliverable: A tight checklist for the very next commit (what code/files change, what I run, what I should see).

Notes log: Create a short "Engineer notes" section to capture open risks, naming, or TODOs.

House rules (hard):

Don't overbuild. Optimize for the next working run.

Prefer explicit data contracts over prose.

No deletion behaviors unless I say so. Always include undo/rollback.

If you're uncertain, state it and bias to the safer, more inspectable choice.

Context you can use right now:

Excel cockpit (Dashboard, Catalog, Notes, Config; optional ReviewQueue).

Focus model (short vs long), file structure detector (series/patterns), multi-placement via links.

Local-first LLM (Ollama) → only escalate to GPT if ambiguity remains after probes.
...
Example (filled for our file organizer-so you see how it reads)

Goal & Success Criteria

Goal: Auto-route new files using focus + structure rules, visible and editable in Excel, with safe multi-placement and undo.

Success: (1) rows appear in Catalog; (2) final_conf computed with focus lift; (3) proposed destinations populated; (4) "apply" creates moves/links; (5) ChangeLog records inverse ops; (6) nothing deletes.

Assumptions & Questions

Assumptions: Windows NTFS; links allowed; Excel is the control plane; Ollama available; GPT optional.

Questions (answer only if blocking):

Default link type on same drive: hardlink or shortcut?

Global mode: RootOnly, AlignWithMyDocuments, or Hybrid?

Minimum columns you want frozen in Catalog?

Plan A (MVP-now)

Read Catalog → compute final_conf = seed + focus_lift.

Propose destinations for Source/Theme/Project; add multi-placement when secondary ≥ 0.35.

"Apply" script: create folders as needed; hardlink when same drive; write ChangeLog with inverse ops.

Notes tab: append a batch journal row after apply.

Plan B (Upgrade path)

Add light content peek; then Ollama; GPT only if still < threshold.

Add File Structure Detector patterns table (regex + scope + destination templates).

Optional PG backend; Excel remains a view.

Effort Ladder

Low: compute focus lift, build paths, write ChangeLog, hardlinks/shortcuts, Notes row.

Medium: content peek for HTML/PDF title, series detection aggregation, ReviewQueue sheet.

High: GPT fallback, embeddings clustering, Postgres migration.

Interfaces (I/O contracts)

Catalog.xlsx columns (minimal): uuid, original_path, seed_conf, focus_label_candidates, focus_lift_applied, final_conf, destinations, duplication_mode, action.

ChangeLog.csv: uuid, before_path, after_path, action, batch_id, inverse_json, ts.

Config.xlsx: thresholds + link policy.

Notes.xlsx: when, context, note, tags, who.

Test & Rollback

Test on 10 mixed files; verify links, no deletions, log has inverse.

Rollback: replay inverse_json for the last batch.

Next Commit Checklist

 Script to compute final_conf and fill destinations

 Apply runner (move/link + ChangeLog + Notes)

 Config read (thresholds, link policy)

 Dry-run flag

Engineer Notes

Multi-drive linking requires .lnk fallback.

Title sanitization must enforce Windows filename rules.

Keep series together if any member routes.
