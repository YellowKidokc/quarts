...
$dGdt=αG0(1−GGmax)−βS(t)+ηtΨ(t)ΛRJ(t)+σG(t)ξG(t)\frac{dG}{dt} = \alpha G_0 \left(1-\frac{G}{G_{\text{max}}}\right) - \beta S(t) + \eta_t \Psi(t)\Lambda R_J(t) + \sigma_G(t) \xi_G(t)dtdG=αG0(1−GmaxG)−βS(t)+ηtΨ(t)ΛRJ(t)+σG(t)ξG(t)$

Where:

- α=0.1\alpha = 0.1 α=0.1 week−1 (prayer efficacy constant)
- Gmax=10G_{\text{max}} = 10 Gmax=10 GU (maximum grace capacity)
- β=0.05\beta = 0.05 β=0.05 week−1 (sin damping coefficient)
...
$dSdt=γS0e−λRp(t)t−δRp(t)+σS(t)ξS(t)\frac{dS}{dt} = \gamma S_0 e^{-\lambda R_p(t) t} - \delta R_p(t) + \sigma_S(t) \xi_S(t)dtdS=γS0e−λRp(t)t−δRp(t)+σS(t)ξS(t)$

Where:

- γ=0.03\gamma = 0.03 γ=0.03 week−1 (entropy growth rate)
- S0=2.1S_0 = 2.1 S0=2.1 EU (baseline entropy/sin)
- λ=0.2\lambda = 0.2 λ=0.2 (sin decay rate parameter)
- δ=0.2\delta = 0.2 δ=0.2 week−1 (repentance efficacy)
...
$Rp(t)=αpPeff(t)+βpW(t)R_p(t) = \alpha_p P_{\text{eff}}(t) + \beta_p W(t)Rp(t)=αpPeff(t)+βpW(t)$
...
$Peff(t)=P(t)+αm∑k=1min⁡(t,4)P(t−k)e−k/τpP_{\text{eff}}(t) = P(t) + \alpha_m \sum_{k=1}^{\min(t,4)} P(t-k) e^{-k/\tau_p}Peff(t)=P(t)+αmk=1∑min(t,4)P(t−k)e−k/τp$
...
$Λ=1G0\Lambda = \frac{1}{\sqrt{G_0}}Λ=G01$
...
- K(t)=1τee−t/τeK(t) = \frac{1}{\tau_e}e^{-t/\tau_e} K(t)=τe1e−t/τe (experience kernel function)
...
1. **Spiritual Growth Number (SGN)**:
    ...
    where ε=10−6\epsilon = 10^{-6} ε=10−6 prevents division by zero
...
E(t)=G(t)+S(t)+∑iFi(t)E(t)=G(t)+S(t)+∑iFi(t)
...
dFidt+=∑j≠iaij(Fj−Fi)dtdFi+=∑j=iaij(Fj−Fi)  
where:
